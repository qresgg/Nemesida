...
public class PlasmaSpheresActivation : MonoBehaviour
{
    ...
    public void MoveInOrbits()
    {
        if (player_AbilityUser != null)
        {
            ...
            if (activeProjectileCount > 0)
            {
                ...
                currentAngle += Time.deltaTime * PlasmaSpheres.ProjectileSpeed * 30;
                ...
                for (int i = 0; i < activeProjectileCount; i++)
                {
                    ...
                    float x = _player.transform.position.x + Mathf.Cos(angle * Mathf.Deg2Rad) * PlasmaSpheres.Radius;
                    float y = _player.transform.position.y + Mathf.Sin(angle * Mathf.Deg2Rad) * PlasmaSpheres.Radius;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            other.gameObject.GetComponent<Enemy>().TakeDamage(PlasmaSpheres.DamageCount);
        }
    }
}
